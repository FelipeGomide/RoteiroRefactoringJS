...
function calcularTotalApresentação(pecas, apre) {
  let total = 0
...
function calcularTotalFatura(pecas, apresentacoes){
  ...
  for (let apre of apresentacoes) {
    totalFatura += calcularTotalApresentação(pecas, apre)
  }
  return totalFatura;
}
...
function gerarFaturaStr (fatura, pecas) { 
    ...
    for (let apre of fatura.apresentacoes) {
      faturaStr += `  ${getPeca(pecas, apre).nome}: ${formatarMoeda(calcularTotalApresentação(pecas, apre))} (${apre.audiencia} assentos)\n`;
    }
...
function gerarFaturaHTML(fatura, pecas) {
  ...
  for (let apre of fatura.apresentacoes) {
    faturaHTML += `<li>  ${getPeca(pecas, apre).nome}: ${formatarMoeda(calcularTotalApresentação(pecas, apre))} (${apre.audiencia} assentos) </li>\n`;
  }
