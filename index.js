...
class ServicoCalculoFatura {
  ...
  calcularTotalApresentação(pecas, apre) {
    let total = 0
  ...
  calcularTotalFatura(pecas, apresentacoes){
    ...
    for (let apre of apresentacoes) {
      totalFatura += this.calcularTotalApresentação(pecas, apre)
    }
    return totalFatura;
  }
...
function gerarFaturaStr (fatura, pecas, calc) { 
  ...
  for (let apre of fatura.apresentacoes) {
    faturaStr += `  ${getPeca(pecas, apre).nome}: ${formatarMoeda(calc.calcularTotalApresentação(pecas, apre))} (${apre.audiencia} assentos)\n`;
  }
...
//   for (let apre of fatura.apresentacoes) {
//     faturaHTML += `<li>  ${getPeca(pecas, apre).nome}: ${formatarMoeda(calc.calcularTotalApresentação(pecas, apre))} (${apre.audiencia} assentos) </li>\n`;
//   }
